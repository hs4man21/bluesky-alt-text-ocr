...
async function getNotifications(): Promise<AppBskyNotificationListNotifications.Notification[]> {
  if (!agent) {
...
function getAuxImage(imageNumber, totalImages, locale, num, totalAlts) {
  ...
  ctx.fillText(
    `Image ${imageNumber} of ${totalImages}, text ${num} of ${totalAlts}`,
    auxImageEdgeLength - 20,
    auxImageEdgeLength - 20,
  );
...
async function ocr(url: any) {
  ...
  if (result && result.textAnnotations) {
    ...
    const localeAndCount = Object.entries(locales).sort((entryA: any, entryB: any) => entryA[1] - entryB[1])[0] || [
      'default',
      0,
    ];
...
function splitText(text: string, maxLen) {
  let result: string[] = [];
  let lastSpan = { end: 0 };
  let lenBase = 0;
  let split = Array.from(splitter.findSpans(text));
  split.forEach((span: any) => {
...
function getRootCdiAndUri(notif: AppBskyNotificationListNotifications.Notification) {
  return {
    cid:
      (
        notif?.record as {
...
async function pollApi(): Promise<void> {
  try {
    ...
    const notifs: AppBskyNotificationListNotifications.Notification[] = await getNotifications();
    ...
    for await (const notif of notifs) {
      ...
      const parentPost = (notif?.record as any)?.reply.parent;

      const parentThread = await agent.api.app.bsky.feed.getPostThread({ uri: parentPost.uri });

      const imagesToOCR = (parentThread?.data?.thread?.post as any)?.record?.embed?.images;
      ...
      let ocrs = await ocrSkeetImages(imagesToOCR, (parentThread?.data?.thread?.post as any)?.author?.did);
      ...
      if (ocrs && ocrs.length > 0) {
        ...
        for await (const ocrRecord of splitOcrs) {
          ...
          if (imageRecord) {
            for (let j = 0; j < ocrRecord.split.length; j++) {
              let auxImage = getAuxImage(
                imageNumber,
                splitOcrs.length,
                ocrRecord.locale,
                j + 1,
                ocrRecord.split.length,
              );
        ...
        let totalImagesToUpload = imageGroups.map((group: any) => group.length).reduce((prev, cur) => prev + cur);
          ...
          if (totalImagesToUpload <= 4) {
            ...
            for (let idxStart = 0; idxStart < group.length; idxStart += 4) {
              ...
              const postRes = await agent.post({
                text: `Alt text ${(idxStart + 1).toString()} through ${
                  group.length < idxStart + 4 ? group.length : idxStart + 4
                } of ${group.length}`,
                reply: currReplyRef,
